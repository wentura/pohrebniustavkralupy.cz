export default function Breadcrumbs({ breads }) {
  // let breads = [
  //   ["/sluzby/pohrby", "Pohřby"],
  //   ["/", "Služby"],
  // ];
  breads ? breads : (breads = []);
  let delkaBreads = breads.length;
  if (delkaBreads == 0) {
    return (
      <nav aria-label="breadcrumb" className="w-full p-4 text-sm hidden">
    ...
    return (
      <nav aria-label="breadcrumb" className="w-full p-4 text-sm">
        <ol className="flex h-8 space-x-2">
          ...
          {breads.map((bread) => (
            <li className="flex items-center space-x-2" key={bread.title}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 32 32"
                aria-hidden="true"
